...
To [fetch](https://docs.commercelayer.io/developers/fetching-resources) a single return line item, send a `GET` request to the `/api/return_line_items/:id` endpoint, where `id` is the ID of the resource that you want to retrieve.
