...
To [fetch](https://docs.commercelayer.io/developers/fetching-resources) a single fixed amount promotion, send a `GET` request to the `/api/fixed_amount_promotions/:id` endpoint, where `id` is the ID of the resource that you want to retrieve.
